              ...
              case 'MASTER':
                if (_event.type == 'story') {
                  ...
                  if (data.text.match(/SYSTEM COMMAND to "Action here-XXX"/i)) {
                    ...
                    let EIV_JSON = data.target.custom_fields.filter(
                      response => {
                    ...
                    let EIV_curr_JSON2 = data.target.custom_fields.data.id.filter(
                      response => {
                        return response.name === EIV_curr_id;
                      }
                    );
                    console.log('EIV_curr_JSON2: ' + EIV_curr_JSON2);

                    let EIV_value = EIV_curr_JSON2[0].name;
                    console.log('EIV_value: ' + EIV_value);

                    let logString;
                    logString = JSON.stringify(EIV_value);
                    logString = JSON.stringify(EIV_value, null, 4); // (Optional) beautiful indented output.
